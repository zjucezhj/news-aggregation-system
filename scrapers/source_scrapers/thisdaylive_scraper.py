...
class ThisDayLiveScraper(BaseScraper):
    """This Day Live 新闻网站爬取类"""
    
    def __init__(self):
        super().__init__("This Day Live")
    ...
    def get_article_list(self, url: str) -> List[Dict]:
        ...
        try:
            ...
            # 策略2：从具有article类的div中提取
            if len(articles) < 30:  # 如果文章数量较少，继续搜索
                div_articles = soup.find_all('div', class_=lambda x: x and 'article' in x.lower())
                print(f"  策略2：找到 {len(div_articles)} 个article类div")
                
                for div_elem in div_articles:
                    links = div_elem.find_all('a', href=True)
                    for link in links:
                        href = link['href']
                        title = link.get_text().strip()
            ...
            # 策略3：从所有链接中提取（针对主页）
            if len(articles) < 50:  # 如果还是不够，搜索所有链接
                ...
                for link in all_links:
                    ...
                    if (title and len(title) > 15 and 
                        ...
                        not any(exclude in href.lower() for exclude in ['category', 'tag', 'author', 'search', 'contact'])):
            ...
            # 策略4：从内容区域提取
            if len(articles) < 50:
                content_areas = soup.find_all('div', class_=lambda x: x and 'content' in x.lower())
                print(f"  策略4：分析 {len(content_areas)} 个内容区域")
            ...
            return articles  # 返回所有找到的文章
            
        except Exception as e:
            print(f"获取This Day Live文章列表失败: {e}")
            return []
    ...
    def extract_article_content(self, url: str) -> Dict:
        ...
        try:
            ...
            print(f"提取This Day Live文章内容失败: {e}")
    ...
    def _extract_content(self, soup: BeautifulSoup) -> str:
        ...
        # 如果通用选择器失败，尝试从主内容区域提取段落
        main_element = soup.find(class_=lambda x: x and 'wp-singular' in x if x else False)
        if main_element:
            ...
            for p in paragraphs:
                ...
                # 过滤掉导航、广告等无关内容
                if (text and 
                    len(text) > 20 and  # 过滤太短的文本
                    not text.lower().startswith('share') and  # 过滤分享按钮
                    not text.lower().startswith('click') and  # 过滤点击提示
                    'advertisement' not in text.lower()):  # 过滤广告
